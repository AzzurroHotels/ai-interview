...
async function measurePing() {
  // Ping your own Supabase endpoint — guaranteed no CORS issues
  const url = SUPABASE_URL + "/rest/v1/?_=";
  const times = [];
  for (let i = 0; i < 5; i++) {
    const t0 = performance.now();
    await fetch(url + (Date.now() + i), {
      cache: "no-store",
  ...
  times.sort((a, b) => a - b);
  return Math.round((times[1] + times[2] + times[3]) / 3);
}

async function measureDownload() {
  // Upload a 2 MB blob to Supabase Storage then download it back
  const size = 2 * 1024 * 1024;
  const payload = new Uint8Array(size);
  crypto.getRandomValues(payload.subarray(0, 4096)); // prevent compression

  const tmpPath = "speedtest/dl_" + Date.now() + ".bin";
  const { error: upErr } = await supabase.storage
    .from("interviews")
    .upload(tmpPath, payload, { contentType: "application/octet-stream", upsert: true });
  if (upErr) throw new Error("DL test prep failed: " + upErr.message);

  const { data: signed, error: signErr } = await supabase.storage
    .from("interviews")
    .createSignedUrl(tmpPath, 60);
  if (signErr) throw new Error("DL signed URL failed: " + signErr.message);
...
async function measureUpload() {
  // Upload 1 MB blob to Supabase Storage and time it
...
function speedRating(ping, down, up) {
  if (down >= 10 && up >= 3 && ping <= 80)  return { label: "Excellent", color: "#16a34a", bg: "rgba(22,163,74,0.08)",   border: "rgba(22,163,74,0.25)" };
  if (down >= 5  && up >= 1 && ping <= 150) return { label: "Good",      color: "#0b1b2b", bg: "rgba(29,139,255,0.07)", border: "rgba(29,139,255,0.18)" };
  if (down >= 2  && up >= 0.5)              return { label: "Fair",      color: "#92400e", bg: "rgba(251,191,36,0.10)", border: "rgba(251,191,36,0.35)" };
