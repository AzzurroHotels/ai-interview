...
async function measureDownload() {
  // Use Cloudflare's speed test CDN — has a Manila PoP, accurate for PH connections
  // Run 3 parallel 10MB downloads to saturate the connection
  const SIZE = 10 * 1024 * 1024;
  const urls = [
    `https://speed.cloudflare.com/__down?bytes=${SIZE}&_=${Date.now()}`,
    `https://speed.cloudflare.com/__down?bytes=${SIZE}&_=${Date.now() + 1}`,
    `https://speed.cloudflare.com/__down?bytes=${SIZE}&_=${Date.now() + 2}`,
  ];
  ...
  const mbps = parseFloat((((totalBytes * 8) / (elapsed_ms / 1000)) / 1_000_000).toFixed(2));
  return Math.min(mbps, MAX_MBPS);
}

async function measureUpload() {
  // Use Cloudflare's upload endpoint — same Manila PoP
  const SIZE = 10 * 1024 * 1024;
  ...
  const elapsed_ms = performance.now() - t0;
  const mbps = parseFloat((((SIZE * 8) / (elapsed_ms / 1000)) / 1_000_000).toFixed(2));
  return Math.min(mbps, MAX_MBPS);
}
