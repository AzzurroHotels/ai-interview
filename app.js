...
async function measureUpload() {
  // Upload to Supabase Storage — this IS the actual destination for videos
  // so it's the most relevant upload metric for this interview app
  const size = 1 * 1024 * 1024; // 1 MB
  const payload = new Uint8Array(size);
  crypto.getRandomValues(payload.subarray(0, 4096)); // prevent compression

  const tmpPath = "speedtest/up_" + Date.now() + ".bin";
  const t0 = performance.now();
  const { error: upErr } = await supabase.storage
    .from("interviews")
    .upload(tmpPath, payload, { contentType: "application/octet-stream", upsert: true });
  const elapsed = (performance.now() - t0) / 1000;

  if (upErr) throw new Error("Upload test failed: " + upErr.message);

  supabase.storage.from("interviews").remove([tmpPath]).catch(() => {});

  return parseFloat(((size * 8) / elapsed / 1e6).toFixed(2));
}
...
async function runSpeedTest() {
  ...
  try {
    ...
    speedTestResults = { ping_ms: null, download_mbps: null, upload_mbps: null, rating: "Test failed" };
    stProgress.textContent = "Test incomplete";
    setMsg("Could not complete speed test. You can still continue.");
    setBar(100);
    continueBtn.disabled = false;
  }
}
