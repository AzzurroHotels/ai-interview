...
async function measureDownload() {
  ...
  // Read the full binary payload so we time the actual transfer
  const buffer = await res.arrayBuffer();
  const elapsed_ms = performance.now() - t0;
  const bytes = buffer.byteLength;
  if (!bytes) throw new Error("Download test failed: empty response");
  const download_mbps = parseFloat(
    (((bytes * 8) / (elapsed_ms / 1000)) / 1_000_000).toFixed(2)
  );
...
async function measureUpload() {
  const base = getSpeedTestBase();
  // Use 4 MB for a more reliable measurement on fast Philippine connections
  ...
  // Use client-side timing for accuracy
  const upload_mbps = parseFloat(
    (((size * 8) / (elapsed_ms / 1000)) / 1_000_000).toFixed(2)
  );
  return upload_mbps;
}
