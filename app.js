...
// -------------------------------
// Speed test via Supabase Edge Function proxy
// Avoids all CORS issues — runs server-side close to candidate's region
// Ping/Download use Cloudflare anycast (nearest PoP, accurate for PH)
// Upload is timed by the Edge Function receiving the payload
// -------------------------------
...
async function measurePing() {
  // Ping directly from browser to Google's generate_204 (1-byte response)
  // Google has a Manila PoP so this is accurate for PH — no CORS issue with no-cors mode
  const endpoints = [
  ...
  // Drop 2 highest and 1 lowest (eliminate outliers), average the rest
...
async function measureDownload() {
  ...
  if (!data.ok) throw new Error("Download test failed: " + data.error);
...
async function measureUpload() {
  ...
  const data = await res.json();
  if (!data.ok) throw new Error("Upload test failed: " + data.error);
