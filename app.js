...
// -------------------------------
// Speed test using nearby CDN servers (accurate for PH & SE Asia)
// Ping  → Google 204 endpoint (has POP in Manila/Singapore)
// Down  → jsDelivr CDN (Singapore POP, serves PH well)
// Up    → Supabase Storage (actual upload destination anyway)
// -------------------------------

async function measurePing() {
  // Google's generate_204 is ~1 byte, has Manila/Singapore POPs
  // Use no-cors so browser doesn't block it — we only care about timing
  const endpoints = [
  ...
  for (let round = 0; round < 6; round++) {
    const url = endpoints[round % endpoints.length] + "?_=" + (Date.now() + round);
    const t0 = performance.now();
    await fetch(url, { mode: "no-cors", cache: "no-store" }).catch(() => {});
    times.push(performance.now() - t0);
  }
  times.sort((a, b) => a - b);
  // Drop fastest and slowest, average the middle
  const mid = times.slice(1, -1);
  return Math.round(mid.reduce((a, b) => a + b, 0) / mid.length);
}

async function measureDownload() {
  // jsDelivr has a Singapore PoP — very low latency from Philippines
  // We fetch a known-size file multiple times and average
  const testFiles = [
    // ~500 KB minified library file on jsDelivr
...
async function measureUpload() {
  // Upload to httpbin which accepts POST data — no auth needed, no CORS issues
  // We use multiple small requests and average them for reliability
  const size = 512 * 1024; // 512 KB per request
  ...
  if (results.length === 0) throw new Error("All upload endpoints failed");
...
function speedRating(ping, down, up) {
  // Thresholds tuned for SE Asia / Philippines
  // Ping via Google CDN (Manila/SG POP) so 80ms is realistic for a good PH connection
  // Download via jsDelivr (SG POP) — 5 Mbps is very achievable in PH on decent broadband
  // Upload to Supabase — 1 Mbps is enough for video upload
...
async function runSpeedTest() {
  ...
  try {
    ...
    speedTestResults = { ping_ms: ping, download_mbps: down, upload_mbps: up, rating: rating.label };
    ...
    console.error("Speed test error:", e?.message || e);
    // Even on failure, store partial results if we have them
    speedTestResults = {
      ping_ms: stPing.textContent !== "—" ? parseInt(stPing.textContent) || null : null,
      download_mbps: stDown.textContent !== "—" ? parseFloat(stDown.textContent) || null : null,
      upload_mbps: stUp.textContent !== "—" ? parseFloat(stUp.textContent) || null : null,
      rating: "Test failed (" + (e?.message || "unknown error") + ")",
    };
    stProgress.textContent = "Test incomplete";
    setMsg("Speed test error: " + (e?.message || e) + ". You can still continue.");
    setBar(100);
    continueBtn.disabled = false;
  }
}
